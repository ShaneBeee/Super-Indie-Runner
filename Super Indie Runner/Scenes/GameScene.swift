...
class GameScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        createLayers()
        // Temp thing to fix a bug with animations
        isPaused = true
        isPaused = false
    }
    ...
    func collectCoin(sprite: SKSpriteNode) {
        ...
        if let coinDust = ParticleHelper.addParticleEffect(name: GameConstants.StringConstants.coinDustEmitterKey, particlePositionRage: CGVector(dx: 5.0, dy: 5.0), position: CGPoint.zero) {
    ...
    func addHUD() {
        ...
        hud.position = CGPoint(x: frame.midX, y: frame.maxY - frame.height*0.07)
        ...
        pauseButton.position = CGPoint(x: frame.midX, y: frame.maxY - pauseButton.size.height + 5)
