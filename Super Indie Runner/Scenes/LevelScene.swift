...
class LevelScene: SKScene {
    ...
    func layoutScene(for world: Int) {
        ...
        if (world != 0) {
            let previousWorldButton = SpriteKitButton(defaultButtonImage: GameConstants.StringConstants.playButton, action: buttonHandler(index:), index: 11)
        ...
        if (world < GameConstants.StringConstants.worldBackgroundNames.count - 1) {
            let nextWorldButton = SpriteKitButton(defaultButtonImage: GameConstants.StringConstants.playButton, action: buttonHandler(index:), index: 10)
        ...
        for row in 0..<3 {
            for column in 0..<3 {
                let levelBox = SpriteKitButton(defaultButtonImage: GameConstants.StringConstants.emptyButton, action: buttonHandler(index:), index: level)
    ...
    func createAndShowLevelPopup(for level: Int) {
        ...
        let levelPopup = ScorePopupNode(buttonHandlerDelegate: self, title: "\(world!)-\(level)", level: levelKey, texture: SKTexture(imageNamed: GameConstants.StringConstants.largePopup), score: ScoreManger.getCurrentScore(for: levelKey)[GameConstants.StringConstants.scoreScoreKey]!, coins: 4, animated: false)
