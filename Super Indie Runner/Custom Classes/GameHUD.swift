...
class GameHUD: SKSpriteNode, HUDDelegate {
    ...
    init(with size:CGSize) {
        ...
        coinLabel.position = CGPoint(x: frame.maxX - coinLabel.frame.size.width*2, y: frame.midY-0.1)
        ...
        for i in 0..<3 {
            ...
            emptySlot.position = CGPoint(x: -superCoinCounter.size.width/2 + emptySlot.size.width/2 + CGFloat(i)*superCoinCounter.size.width/2.5 + superCoinCounter.size.width*0.05, y: superCoinCounter.frame.midY)
